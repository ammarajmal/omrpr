...
file = 'Bd1'; 
nP = 47;                   % pn=number of peaks
idc= 1;                     % frequency & damping id channel
%cal1 = 34.303;                 
%cal2 = 32.684;
cal1 = 27;                 
cal2 = 27;
...
data1 = load(file);
data1 = [data1(:,1)*cal1,data1(:,2)*cal2];
data1 = detrend(data1,'constant'); data1 = detrend(data1);

BF_filter = 'wi'; % 'wi' of 'wo'
switch BF_filter
    case 'wo'
        cut = 30;
        [b,a] = butter(9,cut/fn); data = filtfilt(b,a,data1);
    case 'wi'
        f1 = 1.4358; band = 0.1; fl = f1-band; fh = f1+band; 
        [b, a]=fir1(2000,[fl/fn fh/fn]); data = filtfilt(b,a,data1);
end
...
%subplot(313);plot(T1,data11(:,3));grid;legend('Ch3');xlabel('time(sec)');ylabel('Acc(cm/s^2)');ylim([-200 200]);%xlim([0 80]);
...
xlim([0 15]);xlabel('Frequency[Hz]');legend('Ch1','Ch2');
...
xlim([0 15]);xlabel('Frequency[Hz]');ylabel('Amplitude(mm)');legend('Ch1','Ch2');
...
title('');ylabel('Amplitude(mm)');ylim([-40 40]);xlabel('Time (sec)');grid;xlim([0 40]);
